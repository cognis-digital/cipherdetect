...
How **cipherdetect** fits the wider Cognis suite — a composable set of defensive, analytical
tools that all run on your own hardware and speak plain **JSON** (or an OpenAI-compatible
**`/v1`**). `cipherdetect` lives in the **General OSINT / security tool** cluster.
...
| from | relation | to |
|---|---|---|
| `cipherdetect` | JSON findings compose with | [`awesome-cognis`](https://github.com/cognis-digital/awesome-cognis) |
| `cipherdetect` | AI add-ins are served `/v1` by | [`edgemesh`](https://github.com/cognis-digital/edgemesh) (your fleet) |
| `cipherdetect` | findings can be narrated / reasoned over by | [`humind`](https://github.com/cognis-digital/humind) -> [`agentlex`](https://github.com/cognis-digital/agentlex) |
| `cipherdetect` | export to intel formats via | [`stixgen`](https://github.com/cognis-digital/stixgen) (STIX) / [`attackmap`](https://github.com/cognis-digital/attackmap) (ATT&CK) |

## Composition patterns

Everything reads/writes JSON, so tools chain with ordinary pipes; nothing leaves the box.

**1 — chain within the general osint / security tool cluster.** `cipherdetect` output feeds the next tool:
```bash
cipherdetect ... --format json > out.json          # this tool's findings (see `cipherdetect --help`)
...
cipherdetect ...                                         # vision / reasoning add-ins light up
```

**3 — export findings to your SOC's formats.**
```bash
cipherdetect ... --format json | stixgen from-json > bundle.stix.json   # STIX 2.1
cipherdetect ... --format json | attackmap map > attack.json            # ATT&CK techniques
```

**4 — narrate through cognition + agents.** `humind` extracts salience from `cipherdetect`'s
output; `agentlex` holds it as KB facts and fires Horn rules to escalate. `cipherdetect` slots
