...
- **rules/analyzers** apply the heuristics shipped in `cipherdetect/core.py`.
- **score** ranks by severity.
- **MCP server** (`cipherdetect mcp`) exposes `scan` for Cognis.Studio agents.
