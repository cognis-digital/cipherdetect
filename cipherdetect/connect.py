"""Native cognis-connect emit for cipherdetect — forward findings to any platform.

Maps cipherdetect's JSON output to the canonical `Finding` and forwards it via
...
Usage:
    cipherdetect ... --format json | cipherdetect-emit --to stix
    cipherdetect-emit --to slack --url $WEBHOOK --dry-run < findings.json
"""
...
def map_record(rec: dict) -> dict:
    """Tailor a cipherdetect record toward the Finding contract before normalize().
