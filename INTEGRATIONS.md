...
**cipherdetect** plugs into your stack through [`cognis-connect`](https://github.com/cognis-digital/cognis-connect),
...
Once `cipherdetect` emits JSON findings, pipe them straight to a destination — `--dry-run`
previews the exact request without sending:

```bash
cipherdetect ... --format json | cognis-connect emit --to stix   # STIX 2.1 bundle
cipherdetect ... --format json | cognis-connect emit --to slack --url $URL --token $TOK   # Slack
cipherdetect ... --format json | cognis-connect emit --to webhook --url $URL --token $TOK   # generic webhook
```
...
works whatever `cipherdetect` outputs:

```python
from cognis_connect import normalize, stix
findings = [normalize(rec, source="cipherdetect") for rec in records]   # records = your JSON output
